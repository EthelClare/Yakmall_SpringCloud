package com.yakmall.common.utils;

import com.yakmall.common.properties.JwtKeyProperties;
...
public class JwtUtils {
    ...
    public PublicKey getPublicKey() {
        try {
            // 打印调试信息
            log.info("加载 JKS 文件: 路径={}, 别名={}", keyProperties.getKeyLocation(), keyProperties.getAlias());

            KeyStore keyStore = KeyStore.getInstance("JKS");
            ClassPathResource resource = new ClassPathResource(keyProperties.getKeyLocation());
            try (InputStream inputStream = resource.getInputStream()) {
                keyStore.load(inputStream, keyProperties.getStorePass().toCharArray());
                return keyStore.getCertificate(keyProperties.getAlias()).getPublicKey();
            }
        } catch (Exception e) {
            log.error("公钥加载失败", e);
        ...
        try {
            ...
            try (InputStream inputStream = resource.getInputStream()) {
                ...
                return (PrivateKey) keyStore.getKey(
                        ...
                        keyProperties.getKeyPass().toCharArray()
                );
            }
        } catch (Exception e) {
            log.error("私钥加载失败", e);
